    ...
    <div className="App">
        ...
        글제목.map(function(글제목, i){
          return (
            <div className="list" key={i} id={i}>
              <h3 onClick={ ()=>{ 누른제목변경(i); 누른내용변경(i) }} > { 글제목 }
      ...
      <div className="publish">
        <label htmlFor="title">글제목</label>
          <input  name="title" onChange={ (e)=>{ 입력값변경(e.target.value) } } /><br/>
        <label htmlFor="body">상세내용</label>
          <input name="body" onChange={ (e)=>{ 입력값2변경(e.target.value) } } />
      ...
      {
        modal === true ? <Modal 글제목={글제목} 누른제목={누른제목} 상세내용={상세내용} 누른내용={누른내용} /> : null
